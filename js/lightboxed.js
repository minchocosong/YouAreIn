;( function ( $ , window , document , undefined ) {
	...
	function disableScroll() {
		if ( window.addEventListener ) { window.addEventListener( 'DOMMouseScroll' , scrollPreventDefault , {passive: false} ); }
	...
	function enableScroll() {
		if ( window.removeEventListener ) { window.removeEventListener( 'DOMMouseScroll' , scrollPreventDefault , {passive: false} ); }
	...
	$.extend( Lightboxed.prototype , {
		...
		resizeContent : function () {
			...
			this.lightbox.content
				...
				.children()
					.width( jQuery( window ).width() - 1 * this.options.content.spacing )
